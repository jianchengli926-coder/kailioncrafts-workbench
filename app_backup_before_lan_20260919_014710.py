...
"""
        ...
        with cc_m2:
            try:
                ...
                # 第二行：国家 + 来源 + 跟进时间区间
                cF4, cF5, cF6 = st.columns([1, 1, 2])
                with cF4:
                    # 提取所有国家
                    all_countries = sorted(list(set([c.get("country", "") for c in customers if c.get("country")])))
                    country_f = st.selectbox("国家", ["全部"] + all_countries, key="cl_country")
                with cF5:
                    all_sources = sorted(list(set([c.get("source", "") for c in customers if c.get("source")])))
                    source_f = st.selectbox("来源", ["全部"] + all_sources, key="cl_source")
                with cF6:
                    days_f = st.selectbox(
                        "上次跟进时间",
                        ["全部", "7天内", "30天内", "90天内", "超过90天"],
                        key="cl_days"
                    )
                ...
                for c in customers:
                    ...
                    if days_f != "全部":
                        ...
                        if last_fu:
                            try:
                                from datetime import datetime as _dt
                                days_since = (_dt.now() - _dt.strptime(last_fu[:10], "%Y-%m-%d")).days
                                if days_f == "7天内" and days_since > 7:
                                    continue
                                if days_f == "30天内" and days_since > 30:
                                    continue
                                if days_f == "90天内" and days_since > 90:
                                    continue
...
"""
        ...
        with cc_m7:
            ...
            # ===== 顶部统计条 =====
            total_pipeline = sum(v.get("count", 0) for v in pdata.values())
            st.markdown(f"**总客户数：{total_pipeline}**")
            st.markdown("---")
            ...
            for i, stage in enumerate(PIPELINE_STAGES):
                with cols[i]:
                    ...
                    st.markdown(
                        f'<div style="border-top:4px solid {stage["color"]};padding:8px;background:#fafafa;border-radius:8px;">'
                        f'<b>{stage["name"]}</b> <span style="float:right;color:{stage["color"]};font-weight:700;">{v.get("count",0)}</span></div>',
                        unsafe_allow_html=True)
                    for c in v.get("customers", []):
                        g = c.get("grade", "C")
                        st.markdown(
                            f'<div style="background:#fff;border:1px solid #eee;border-left:4px solid {_grade_color.get(g,"#999")};'
                            f'padding:8px;border-radius:6px;margin:6px 0;font-size:12px;">'
                        ...
                        if stage["key"] != "closed":
                            ...
                            if st.button(f"→ {next_stage_name}", key=btn_key, use_container_width=True):
                                ...
                                change_content = f"""# 商机阶段变更记录

**变更时间**：{_dt.now().strftime("%Y-%m-%d %H:%M:%S")}
**客户名称**：{c.get('company_name','')}
**所在国家**：{c.get('country','')}
**变更前阶段**：{old_stage_name}
**变更后阶段**：{next_stage_name}
**客户等级**：{g}级
**客户来源**：{c.get('source','')}
**下次跟进**：{c.get('next_follow_up','未定')}
"""
                                save_to_kb_button(change_content, "客户管理/商机管道", f"阶段变更_{c.get('company_name','')[:20]}_{_dt.now().strftime('%Y%m%d_%H%M')}", "md")
        ...
        with cc_m8:
            ...
            # 分类2：报价后未回（阶段=已报价，且超过3天没跟进）
            quoted_no_reply = []
            for c in customers_all:
                stage_key = c.get("pipeline_stage", "lead")
                if stage_key == "quoted":
                    last_fu = c.get("last_follow_up", "")
                    if last_fu:
                        try:
            ...
            card1.metric("🚨 已超期", len(overdue_list), delta_color="inverse")
            card2.metric("💬 报价未回", len(quoted_no_reply), delta_color="inverse")
            card3.metric("📦 样品待确认", len(sample_wait), delta_color="inverse")
            card4.metric("⭐ A级未跟进", len(a_grade_idle), delta_color="inverse")

            st.markdown("---")

            # ===== 详细列表 =====
            def _render_alert_list(title, icon, items, reason_fn):
                st.markdown(f"**{icon} {title}（{len(items)}）**")
                if items:
                    for c in items:
                        reason = reason_fn(c)
                        st.warning(f"**{c.get('company_name','')}**（{c.get('country','')} · {c.get('grade','C')}级）— {reason}")
                else:
                    st.success("✅ 无")
                st.markdown("")
            ...
            _render_alert_list(
                "已超期未跟进", "🚨",
                overdue_list,
                lambda c: f"计划跟进日 {c.get('next_follow_up','')} 已过"
            )

            _render_alert_list(
                "报价后未回复（≥3天）", "💬",
                quoted_no_reply,
                lambda c: f"报价后 {c.get('days_since',0)} 天未回复"
            )

            _render_alert_list(
                "样品寄出待确认（≥7天）", "📦",
                sample_wait,
                lambda c: f"样品寄出 {c.get('days_since',0)} 天未确认反馈"
            )

            _render_alert_list(
                "A级客户7天未跟进", "⭐",
                a_grade_idle,
                lambda c: f"A级客户已 {c.get('days_since',0)} 天未互动"
            )

            st.markdown("---")

            # ===== 导出到知识库 =====
            st.markdown("**📤 导出提醒清单到知识库**")
            if st.button("💾 导出今日提醒清单", key="export_reminders"):
                from datetime import datetime as _dt
                today_str = _dt.now().strftime("%Y%m%d")
                content = f"""# 客户跟进提醒清单 - {_dt.now().strftime("%Y-%m-%d %H:%M")}

## 🚨 已超期未跟进（{len(overdue_list)}个）
"""
                for c in overdue_list:
                    content += f"- {c.get('company_name','')}（{c.get('country','')}）— 计划跟进日 {c.get('next_follow_up','')}\n"

                content += f"""
## 💬 报价后未回复（{len(quoted_no_reply)}个）
"""
                for c in quoted_no_reply:
                    content += f"- {c.get('company_name','')}（{c.get('country','')}）— 报价后 {c.get('days_since',0)} 天\n"

                content += f"""
## 📦 样品寄出待确认（{len(sample_wait)}个）
"""
                for c in sample_wait:
                    content += f"- {c.get('company_name','')}（{c.get('country','')}）— 样品寄出 {c.get('days_since',0)} 天\n"

                content += f"""
## ⭐ A级客户7天未跟进（{len(a_grade_idle)}个）
"""
                for c in a_grade_idle:
                    content += f"- {c.get('company_name','')}（{c.get('country','')}）— 已 {c.get('days_since',0)} 天未互动\n"

                save_to_kb_button(content, "客户管理/跟进提醒", f"提醒清单_{today_str}", "md")

            with st.expander("📋 智能提醒规则（说明）"):
                st.markdown("""
- **🚨 已超期未跟进**：计划跟进日已过，但还没跟进
- **💬 报价后未回复**：发了报价超过3天，客户没回复
- **📦 样品寄出待确认**：寄了样品超过7天，客户没反馈
- **⭐ A级客户7天未跟进**：A级重点客户，7天没互动容易丢单
...
中文输出，具体可执行，不要空话。"""
            ...
            if _view == "AI作图":
                ...
                if st.button("🚀 生成图片", type="primary", use_container_width=True, key="ais_gen"):
                    st.info("⏳ 已提交生成任务（需在设置中配置生图模型API）")
                ...
                st.markdown("#### 🎨 风格复刻")
                st.markdown("上传参考图，AI解析其视觉风格，应用到你的产品上")
                st.file_uploader("📤 上传参考风格图", type=["png","jpg","jpeg","webp"], key="ais_style_ref")
                st.text_input("产品名称", placeholder="如：不锈钢厨房剪刀", key="ais_style_product")
                if st.button("🔍 分析风格 + 生成", type="primary", key="ais_style_gen"):
                    st.info("⏳ 已提交分析任务")
                ...
                for _t in _TOOL_LIST:
                    with st.expander(f"{_t['name']}", expanded=False):
                        st.write(_t['desc'])
                        st.file_uploader(f"上传图片 for {_t['name']}", type=["png","jpg","jpeg","webp"], key=f"ais_tool_{_t['name']}")
                        if st.button(f"开始{_t['name']}", key=f"ais_tool_btn_{_t['name']}"):
                            st.info("⏳ 处理中...")
