...
中文输出，具体可执行，不要空话。"""
            ...
            # 嵌入本地单文件 HTML 工作台（运行时读取，便于后续维护）
            try:
                from pathlib import Path
                _studio_path = Path(__file__).parent / "tools" / "ai_image_studio.html"
                _studio_html = _studio_path.read_text(encoding="utf-8")
                components.html(_studio_html, height=900, scrolling=True)
            except Exception as _e:
                st.error(f"AI作图工作台加载失败：{_e}")
                st.info("请确认 tools/ai_image_studio.html 文件存在")
